...
Deno.serve(async (req) => {
  ...
  for (const u of users) {
    try {
      ...
      // Ensure single merchant record
      const { data: existingMerchant } = await admin.from("merchants").select("id").eq("user_id", userId).order("created_at", { ascending: true }).limit(1).maybeSingle();
      let merchantId: string;
      if (existingMerchant) {
        merchantId = existingMerchant.id;
        // Update name
        await admin.from("merchants").update({ name: u.merchantName }).eq("id", merchantId);
      } else {
  ...
  return new Response(JSON.stringify({ results, env_url: supabaseUrl }, null, 2), {

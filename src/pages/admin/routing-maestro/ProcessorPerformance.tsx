    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <Card className="p-5">
          ...
          <div className="h-64">
            {procs.length === 0 ? (
              <div className="h-full flex items-center justify-center text-sm text-muted-foreground">No processors configured</div>
            ) : (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {isLoading
            ? Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-48 rounded-lg" />)
            : procs.map((p) => (
                <Card key={p.id} className={`p-5 ${!p.enabled ? "opacity-60" : ""}`}>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Switch checked={p.enabled} onCheckedChange={(v) => toggleProcessor(p.id, v)} />
                  </div>
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-muted-foreground">Success Rate</span>
                        <span className={`font-mono font-medium ${p.successRate >= 95 ? "text-success" : p.successRate >= 90 ? "text-warning" : "text-destructive"}`}>
                          {p.successRate}%
                        </span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="rounded-lg bg-muted/50 p-2.5">
                        ...
                        <p className="text-sm font-semibold font-mono text-foreground">${(p.volume / 1000).toFixed(0)}k</p>
                      </div>
                      <div className="rounded-lg bg-muted/50 p-2.5">
                        <p className="text-xs text-muted-foreground">Failure</p>
                        <p className="text-sm font-semibold font-mono text-destructive">{p.failureRate}%</p>
                      </div>
                      <div className="rounded-lg bg-muted/50 p-2.5">
                        <p className="text-xs text-muted-foreground">Latency</p>
                        <p className="text-sm font-semibold font-mono text-foreground">{p.latency}ms</p>
                      </div>
                      <div className="rounded-lg bg-muted/50 p-2.5">
                        <p className="text-xs text-muted-foreground">Cap Usage</p>
                        <p className="text-sm font-semibold font-mono text-foreground">{((p.currentVolume / p.monthlyCap) * 100).toFixed(0)}%</p>
                      </div>
                    </div>
                  </div>
                </Card>

...
type Merchant = { id: string; name: string; gambling_enabled: boolean | null };
type HRP = {
  merchant_id: string;
...
export default function HighRiskProcessors() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: m }, { data: h }] = await Promise.all([
        supabase.from("merchants").select("id,name,gambling_enabled").order("name").limit(500),
        (supabase as any).from("merchant_high_risk_processors").select("*"),
      ]);
  ...
  const getHrp = (id: string): HRP =>
    hrps[id] || {
      ...
      plgin_enabled: false, vertical: "gambling", notes: null,
    };

  const persist = async (id: string, patch: Partial<HRP>) => {
    const merged = { ...getHrp(id), ...patch, merchant_id: id };
  ...
  const toggleGambling = async (id: string, enabled: boolean) => {
    ...
    const { error } = await (supabase as any).from("merchants").update({ gambling_enabled: enabled }).eq("id", id);
  ...
  return (
    <AppLayout>
      <div className="space-y-6">
        <header>
          <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground">High-Risk Processors</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Enable Matrix Partners, ValensPay PG6, Elektropay, Circoflows and Plgin for merchants in the
            <strong> gambling, casino, sweepstakes and iGaming </strong> verticals only. The vertical gate must
            be on before any high-risk processor will accept traffic; otherwise routing falls back to ShieldHub.
          </p>
        </header>
                  ...
                  <CardHeader className="flex flex-row items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Switch id={`gate-${m.id}`} checked={gateOn} onCheckedChange={(v) => toggleGambling(m.id, v)} />
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                    <div className="space-y-2 rounded-md border p-3">
                      <div className="flex items-center justify-between">
                        <Label className="font-medium">Matrix Partners</Label>
                        <Switch checked={h.matrix_enabled} disabled={!gateOn}
                          onCheckedChange={(v) => persist(m.id, { matrix_enabled: v })} />
                      </div>
                      <Select value={h.matrix_flow} disabled={!gateOn || !h.matrix_enabled}
                        onValueChange={(v) => persist(m.id, { matrix_flow: v as HRP["matrix_flow"] })}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          {MATRIX_FLOWS.map((f) => <SelectItem key={f.value} value={f.value}>{f.label}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    <div className="space-y-2 rounded-md border p-3">
                      <div className="flex items-center justify-between">
                        <Label className="font-medium">ValensPay PG6</Label>
                        <Switch checked={h.valenspay_enabled} disabled={!gateOn}
                          onCheckedChange={(v) => persist(m.id, { valenspay_enabled: v })} />
                      </div>
                      <p className="text-xs text-muted-foreground">HMAC-SHA1 signed v3.2.</p>
                    </div>

                    <div className="space-y-2 rounded-md border p-3">
                      <div className="flex items-center justify-between">
                        <Label className="font-medium">Elektropay (crypto)</Label>
                        <Switch checked={h.elektropay_enabled} disabled={!gateOn}
                          onCheckedChange={(v) => persist(m.id, { elektropay_enabled: v })} />
                      </div>
                      <p className="text-xs text-muted-foreground">USDT / USDC / BTC.</p>
                    </div>

                    <div className="space-y-2 rounded-md border p-3">
                      <div className="flex items-center justify-between">
                        <Label className="font-medium">Circoflows</Label>
                        <Switch checked={h.circoflows_enabled} disabled={!gateOn}
                          onCheckedChange={(v) => persist(m.id, { circoflows_enabled: v })} />
                      </div>
                      <Select value={h.circoflows_mode} disabled={!gateOn || !h.circoflows_enabled}
                        onValueChange={(v) => persist(m.id, { circoflows_mode: v as HRP["circoflows_mode"] })}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          {CIRCOFLOWS_MODES.map((f) => <SelectItem key={f.value} value={f.value}>{f.label}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    <div className="space-y-2 rounded-md border p-3">
                      <div className="flex items-center justify-between">
                        <Label className="font-medium">Plgin (Plugg & Co)</Label>
                        <Switch checked={h.plgin_enabled} disabled={!gateOn}
                          onCheckedChange={(v) => persist(m.id, { plgin_enabled: v })} />
                      </div>
                      <p className="text-xs text-muted-foreground">CreditCard / Visa sandbox.</p>
                    </div>
                  </CardContent>
                </Card>
              );

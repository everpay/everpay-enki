...
export default function AdminCryptoWallets() {
  ...
  const filtered = useMemo(() => wallets.filter((w) => {
    ...
    return true;
  }), [wallets, filter, merchant, asset, status]);
  ...
  return (
    <AppLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <Stat label="Active" value={wallets.filter((w) => w.status === "active").length} />
        <Stat label="Frozen" value={wallets.filter((w) => w.status === "frozen").length} />
      </div>
      <div className="rounded-xl border border-border bg-card">
        <div className="p-4 grid grid-cols-1 md:grid-cols-5 gap-3 border-b border-border">
          <div className="relative md:col-span-2">
            <Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Search asset, address, network..." value={filter} onChange={(e) => setFilter(e.target.value)} className="pl-9" />
          </div>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={w.id}>
                ...
                <TableCell className="text-right text-emerald-500">{Number(w.available).toFixed(6)}</TableCell>
                <TableCell>
                  <Badge variant={w.status === "active" ? "default" : w.status === "frozen" ? "secondary" : "destructive"}>{w.status}</Badge>
                </TableCell>
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Button size="icon" variant="ghost" title="Deposit" onClick={() => { setActionWallet(w); setActionMode("deposit"); }}><ArrowDownToLine className="h-4 w-4" /></Button>
                    <Button size="icon" variant="ghost" title="Withdraw" onClick={() => { setActionWallet(w); setActionMode("withdraw"); }}><ArrowUpFromLine className="h-4 w-4" /></Button>
                    <Button size="icon" variant="ghost" title="Freeze" disabled={w.status !== "active"} onClick={() => freeze.mutate({ wallet_id: w.id })}><Snowflake className="h-4 w-4" /></Button>
                    <Button size="icon" variant="ghost" title="Close" disabled={w.status === "closed"} onClick={() => close.mutate({ wallet_id: w.id })}><X className="h-4 w-4" /></Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
      ...
      <WalletActionDialog wallet={actionWallet} mode={actionMode} onClose={() => { setActionWallet(null); setActionMode(null); }} />
    </AppLayout>
  );
}

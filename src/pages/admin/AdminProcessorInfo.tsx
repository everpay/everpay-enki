...
const processors: ProcessorConfig[] = [
  ...
  },
  {
    id: 'facilitapay',
    name: 'facilitapay',
    displayName: 'FacilitaPay (Attrus)',
    status: 'live',
    statusLabel: 'Live — LATAM Processing',
    description: 'Unified LATAM payment processing via Attrus API. PIX, Boleto, TED, SPEI, PSE, Credit/Debit Cards. Brazil, Mexico, Colombia. Settlement via SWIFT or Stablecoin (USDC/USDT).',
    regions: ['BR', 'MX', 'CO'],
    currencies: ['BRL', 'MXN', 'COP', 'USD'],
    apiEndpoint: 'api.facilitapay.com/api/v1',
    docsUrl: 'https://docs.attrus.com/production',
    contractEntity: 'Facilita Inc., Miami FL / Nova Lima MG, Brazil',
    authMethod: 'Bearer Token (email + password → /login)',
    settlementTerms: 'SWIFT — FREE for transfers >$5k USD ($45 fee if <$5k). USDC/USDT +0.20%. Overnight conversion +0.35%.',
    rollingReserve: 'None specified',
    icon: <Banknote className="h-5 w-5 text-emerald-500" />,
    endpoints: [
      { method: 'POST', path: '/api/v1/login', desc: 'Authenticate & get Bearer token', status: 'live' },
      { method: 'POST', path: '/api/v1/transactions/brazil', desc: 'Create Brazil transaction (PIX, Boleto, TED, Card)', status: 'live' },
      { method: 'POST', path: '/api/v1/transactions/mexico', desc: 'Create Mexico transaction (SPEI, Card)', status: 'live' },
      { method: 'POST', path: '/api/v1/transactions/colombia', desc: 'Create Colombia transaction (PSE, Card)', status: 'live' },
      { method: 'POST', path: '/api/v1/transactions/card', desc: 'Cross-border card transaction (3DS)', status: 'live' },
      { method: 'GET', path: '/api/v1/transactions/:id', desc: 'Get transaction status', status: 'live' },
      { method: 'POST', path: '/api/v1/company', desc: 'Register company (KYB)', status: 'live' },
      { method: 'POST', path: '/api/v1/person', desc: 'Register person (KYC)', status: 'live' },
      { method: 'POST', path: '/api/v1/payouts', desc: 'Create payout (PIX, TED, SPEI)', status: 'live' },
      { method: 'GET', path: '/api/v1/internal-bank-accounts', desc: 'List internal bank accounts', status: 'live' },
      { method: 'POST', path: '/api/v1/subscriptions', desc: 'Create recurring subscription', status: 'live' },
    ],
    fees: [
      // Brazil

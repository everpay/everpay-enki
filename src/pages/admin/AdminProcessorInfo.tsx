...
const processors: ProcessorConfig[] = [
  ...
  },
  {
    ...
    status: 'active',
    statusLabel: 'Active — APAC Card Acquiring',
    description: 'Asian card acquiring (Visa / Mastercard / AmEx / JCB / Diners / Discover) with tiered MDR by monthly volume. 10% / 180-day rolling reserve.',
    regions: ['ASIA', 'CN', 'HK', 'JP', 'KR', 'SG', 'TW', 'TH', 'VN', 'MY', 'ID', 'PH'],
    currencies: ['USD', 'HKD', 'SGD', 'JPY', 'CNY'],
    apiEndpoint: 'https://api.carespay.com',
    authMethod: 'API Key + HMAC',
    settlementTerms: 'Visa/MC: T+7 → T+5 (≥50k 7d avg) → T+3 (≥100k 7d avg); daily if monthly ≥3M. AE/JCB: T+7.',
    rollingReserve: '10% / 180 days',
    contractEntity: 'Carespay Ltd.',
    icon: <CreditCard className="h-5 w-5" />,
    endpoints: [
      { method: 'POST', path: '/v1/payments', desc: 'Create card payment', status: 'live' },
      { method: 'POST', path: '/v1/refunds', desc: 'Refund a payment', status: 'live' },
      { method: 'GET',  path: '/v1/transactions/{id}', desc: 'Fetch transaction', status: 'live' },
    ],
    fees: [
      { id: 'cp-setup', method: 'Setup Fee (one-time)', fee: '$1,000 USD', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-annual', method: 'Annual Service Fee', fee: '$1,000 USD', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-mdr-mc-low', method: 'MDR — MC/Diners/Discover (<3M USD/M)', fee: '5.00%', markupType: 'percentage', markupValue: 0 },
      { id: 'cp-mdr-mc-hi',  method: 'MDR — MC/Diners/Discover (≥3M USD/M)', fee: '4.50%', markupType: 'percentage', markupValue: 0 },
      { id: 'cp-mdr-visa-low', method: 'MDR — Visa (<1.5M USD/M)', fee: '6.50%', markupType: 'percentage', markupValue: 0 },
      { id: 'cp-mdr-visa-hi',  method: 'MDR — Visa (≥1.5M USD/M)', fee: '6.00%', markupType: 'percentage', markupValue: 0 },
      { id: 'cp-mdr-aejcb',    method: 'MDR — AmEx / JCB', fee: '7.00%', markupType: 'percentage', markupValue: 0 },
      { id: 'cp-cb-mc-low',    method: 'Chargeback — MC/Diners/Discover/JCB (<0.9%)', fee: '$30', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-cb-mc-mid',    method: 'Chargeback — MC/Diners/Discover/JCB (0.9–2%)', fee: '$50', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-cb-mc-hi',     method: 'Chargeback — MC/Diners/Discover/JCB (≥2%)', fee: '$150', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-cb-ae-low',    method: 'Chargeback — AmEx (<0.9%)', fee: '$50', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-cb-ae-mid',    method: 'Chargeback — AmEx (0.9–2%)', fee: '$80', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-cb-ae-hi',     method: 'Chargeback — AmEx (≥2%)', fee: '$150', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-cb-visa-low',  method: 'Chargeback — Visa (<0.65%)', fee: '$30', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-cb-visa-mid',  method: 'Chargeback — Visa (0.65–0.75%)', fee: '$50', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-cb-visa-hi',   method: 'Chargeback — Visa (≥0.75%)', fee: '$150', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-refund',       method: 'Refund Processing Fee', fee: '$3 / refund', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-tx-mc',        method: 'Per Transaction — Master/Visa/Diners/JCB/Discover (approved & declined)', fee: '$0.30', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-tx-ae',        method: 'Per Transaction — AmEx (approved & declined)', fee: '$0.50', markupType: 'fixed', markupValue: 0 },
      { id: 'cp-reserve',      method: 'Rolling Reserve', fee: '10% / 180 days', markupType: 'percentage', markupValue: 0 },
      { id: 'cp-wire',         method: 'International Wire — USDT', fee: '1.5%', markupType: 'percentage', markupValue: 0 },
    ],
  },
];

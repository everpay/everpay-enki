...
export default function Subscriptions() {
  ...
  const [billingInterval, setBillingInterval] = useState('4');
  const [billingPeriodUnit, setBillingPeriodUnit] = useState('weeks');
  ...
  const [priceRows, setPriceRows] = useState<PriceRow[]>([
    { id: crypto.randomUUID(), currency: 'EUR', subscription_price: '', trial_price: '0', is_default: true },
  ]);
  ...
  const resetForm = () => {
    ...
    setBillingInterval('4'); setBillingPeriodUnit('weeks'); setEndsType('never');
    setEndsAfterCount('1'); setEndsAfterUnit('year'); setTrialEnabled(false);
    setTrialDuration('1'); setTrialUnit('month'); setRetryLogic('4_retries_1d_fri_2d_5d');
    setPriceRows([{ id: crypto.randomUUID(), currency: 'EUR', subscription_price: '', trial_price: '0', is_default: true }]);
  };
  ...
  const handleCreatePlan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Insert multi-currency price rows
  ...
  const addPriceRow = () => {
    setPriceRows(prev => [...prev, {
      id: crypto.randomUUID(), currency: 'USD', subscription_price: '', trial_price: '0', is_default: false,
    }]);
  };
  ...
  const selectedRetry = retryOptions.find(r => r.value === retryLogic);
    ...
    <AppLayout>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList className="bg-transparent border-b border-border rounded-none p-0 h-auto gap-0">
          {['Plans', 'Subscriptions', 'PSP', 'Oneclicks', 'Tokens'].map(tab => (
            <TabsTrigger
              key={tab}
          ...
          {!showCreateForm ? (
            <>
              ...
              <div className="flex flex-wrap items-center gap-2">
                {['Plan ID', 'Name', 'Default currency', 'Trial', 'Created', 'Status'].map(f => (
                  <Badge key={f} variant="outline" className="cursor-pointer hover:bg-muted px-3 py-1 text-xs font-normal">
                    {f} <span className="ml-1 text-muted-foreground">▾</span>
                  </Badge>
                ))}
              ...
              <CardContent>
                <form onSubmit={handleCreatePlan} className="space-y-6 max-w-2xl">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Textarea value={planDescription} onChange={e => setPlanDescription(e.target.value)} rows={4} />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <RadioGroup value={subscriptionStarts} onValueChange={setSubscriptionStarts} className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Label htmlFor="start-wd" className="font-normal text-sm">on the nearest</Label>
                  ...
                  <div className="space-y-2">
                    ...
                    <RadioGroup value={endsType} onValueChange={setEndsType} className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Label htmlFor="ends-after" className="font-normal text-sm">After period</Label>
                          ...
                          <TableRow key={row.id}>
                            ...
                            <TableCell>
                              <Select value={row.currency} onValueChange={v => updatePriceRow(row.id, 'currency', v)}>
                                ...
                                <SelectContent>
                                  {['EUR','USD','GBP','BRL','CAD','AUD','JPY','CHF','SEK','NOK','DKK','PLN','CZK','HUF','RON','BGN','HRK','TRY','MXN','ARS','CLP','COP','PEN','UYU','ZAR','KES','NGN','GHS','EGP','MAD','AED','SAR','QAR','KWD','BHD','OMR','INR','PKR','BDT','LKR','THB','MYR','SGD','IDR','PHP','VND','KRW','TWD','HKD','CNY','NZD'].map(c => (
        ...
        <TabsContent value="psp">
          <Card>
            <CardContent className="py-12 text-center text-muted-foreground">
              PSP configuration for subscription processing. Connect payment providers for recurring billing.
            </CardContent>
          </Card>
        </TabsContent>

        {/* ── ONECLICKS TAB ── */}
        <TabsContent value="oneclicks">
          <Card>
            <CardContent className="py-12 text-center text-muted-foreground">
              One-click payment tokens for returning subscribers.
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="tokens">
          <Card>
            <CardContent className="py-12 text-center text-muted-foreground">
              Tokenized card data for recurring payments and saved payment methods.
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>

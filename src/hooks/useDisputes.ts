...
export function useDisputes() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from('disputes').select('*').eq('merchant_id', merchant.id).order('created_at', { ascending: false });
...
export function useDisputeStats(disputes: Dispute[]) {
  ...
  const recoveredAmount = disputes.filter(d => d.outcome === 'won' || d.status === 'won').reduce((sum, d) => sum + d.amount, 0);

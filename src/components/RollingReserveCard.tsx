...
export function RollingReserveCard() {
  ...
  const heldByCurrency = heldReserves.reduce((acc, r) => { acc[r.currency] = (acc[r.currency] || 0) + Number(r.amount); return acc; }, {} as Record<string, number>);

  return (
    <Card className="border-border bg-card shadow-card">
      <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Shield className="h-4 w-4 text-amber-500" />Rolling Reserve (10%)</CardTitle></CardHeader>
      <CardContent className="space-y-4">
        {isLoading ? <p className="text-sm text-muted-foreground">Loading...</p> : reserves.length === 0 ? (
          <p className="text-sm text-muted-foreground">No reserves yet. Reserves are created on successful payments.</p>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg bg-amber-500/10 p-3"><div className="flex items-center gap-1.5 text-xs text-amber-600 dark:text-amber-400"><Clock className="h-3 w-3" />Held</div><p className="mt-1 text-lg font-bold text-foreground">${totalHeld.toFixed(2)}</p><p className="text-xs text-muted-foreground">{heldReserves.length} reserves</p></div>

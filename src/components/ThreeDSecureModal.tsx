    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[85vh] p-0 overflow-hidden">
        ...
        <div className="relative w-full" style={{ height: '500px' }}>
          ...
          <iframe src={redirectUrl} className="w-full h-full border-0" onLoad={() => setLoading(false)} sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-top-navigation" title="3D Secure Authentication" />
        </div>
        <div className="p-3 border-t border-border flex items-center justify-between bg-muted/30">
          ...
          <Button variant="ghost" size="sm" className="gap-1 text-xs" onClick={() => window.open(redirectUrl, '_blank', 'width=500,height=700,scrollbars=yes')}>

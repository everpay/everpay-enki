...
export function getTransactionStatusInfo(
  ...
  const rawCode = providerResponse.error?.code || providerResponse.statusCode || providerResponse.respcode || providerResponse.response_code || '';
...
function extractDeclineReason(message: string, code: string, providerResponse: any): string {
  // Try to get a meaningful decline reason from various processor response formats
  if (message && message !== 'Transaction declined' && message !== 'Declined') {
    return message;
  }

  // Map common response codes to human-readable reasons
